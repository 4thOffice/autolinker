...
var parse_html_1 = require("../../src/htmlParser/parse-html");
describe("Autolinker.htmlParser.HtmlParser", function () {
     ...
     */
    function parseHtmlAndCapture(html) {
        var parsedNodes = [];
        parse_html_1.parseHtml(html, {
            onOpenTag: function (tagName, offset) {
                parsedNodes.push({ type: 'openTag', tagName: tagName, offset: offset });
            },
            onText: function (text, offset) {
                parsedNodes.push({ type: 'text', text: text, offset: offset });
            },
            onCloseTag: function (tagName, offset) {
                parsedNodes.push({ type: 'closeTag', tagName: tagName, offset: offset });
            },
            onComment: function (offset) {
                parsedNodes.push({ type: 'comment', offset: offset });
            },
            onDoctype: function (offset) {
                parsedNodes.push({ type: 'doctype', offset: offset });
            }
        });
        return parsedNodes;
    }
    describe('text node handling', function () {
        it("when provided an empty string, should not emit any nodes", function () {
            var nodes = parseHtmlAndCapture('');
            expect(nodes).toEqual([]);
        });
        it("should return a single text node if there are no HTML nodes in it", function () {
            var nodes = parseHtmlAndCapture("Testing 123");
    ...
    describe('HTML element node handling', function () {
        ...
        it("when there are multiple html elements, should parse each of them", function () {
            var nodes = parseHtmlAndCapture("Test <div>b<span> Test");
            expect(nodes).toEqual([
                { type: 'text', text: 'Test ', offset: 0 },
                { type: 'openTag', tagName: 'div', offset: 5 },
        ...
        it("when there are end tags in the string, should parse them correctly", function () {
            var nodes = parseHtmlAndCapture("Test <div>b</div> <span>Test</span>");
            expect(nodes).toEqual([
                { type: 'text', text: 'Test ', offset: 0 },
                { type: 'openTag', tagName: 'div', offset: 5 },
                { type: 'text', text: 'b', offset: 10 },
        ...
        it("when a string starts with html, should parse correctly", function () {
            var nodes = parseHtmlAndCapture("<div>Test</div> Test2");
            expect(nodes).toEqual([
                { type: 'openTag', tagName: 'div', offset: 0 },
        ...
        it("should handle html tags with attributes", function () {
            var nodes = parseHtmlAndCapture("<div id=\"hi\" class='some-class' align=center>Test</div>");
            expect(nodes).toEqual([
                { type: 'openTag', tagName: 'div', offset: 0 },
        ...
        it("should handle html tags with unqouted attributes", function () {
            var nodes = parseHtmlAndCapture("<div id=hi class=some-class align=center>Test</div>");
            expect(nodes).toEqual([
                { type: 'openTag', tagName: 'div', offset: 0 },
        ...
        it("should handle anchor tags with unqouted href's and query strings (Issue #263)", function () {
            var nodes = parseHtmlAndCapture("<a href=https://www.facebook.com/hashtag/muntcentrum?epa=HASHTAG>#Muntcentrum</a>");
            expect(nodes).toEqual([
        ...
        it("when there are two << characters, the first one should be ignored\n\t\t\t if the second one forms a tag", function () {
            var nodes = parseHtmlAndCapture("<<div>Test</div>");
            expect(nodes).toEqual([
        ...
        it("when we have text such as '<xyz<', the first '<' should be ignored\n\t\t     if the second one forms a tag", function () {
            var nodes = parseHtmlAndCapture("<xyz<div>Test</div>");
            expect(nodes).toEqual([
        ...
        it("when we have text such as '<xyz<', and the second '<' does not form\n\t\t     a tag, this sequence should be treated as text", function () {
            var nodes = parseHtmlAndCapture("<xyz< Test <3<asdf");
            expect(nodes).toEqual([
                { type: 'text', text: '<xyz< Test <3<asdf', offset: 0 }
            ]);
        });
        it("should handle some more complex HTML strings", function () {
            var nodes = parseHtmlAndCapture('Joe went to <a href="google.com">ebay.com</a> today,&nbsp;and bought <b>big</b> items');
            expect(nodes).toEqual([
        ...
        it("should properly handle a tag where the attributes start on the " +
            "next line", function () {
            var nodes = parseHtmlAndCapture('Test <div\nclass="myClass"\nstyle="color:red"> Test');
            expect(nodes).toEqual([
                { type: 'text', text: 'Test ', offset: 0 },
                { type: 'openTag', tagName: 'div', offset: 5 },
        ...
        it("should properly handle xml namespaced elements", function () {
            var nodes = parseHtmlAndCapture('<ns:p>Go to google.com or <a data-qux-="test" href="http://www.example.com">Bar</a> Baz</ns:p>');
            expect(nodes).toEqual([
    ...
    describe('HTML comment node handling', function () {
        it("should properly handle text that starts with the sequence '<!' but\n\t\t     doesn't form a comment tag", function () {
            var nodes = parseHtmlAndCapture('<! Hello');
            expect(nodes).toEqual([
        ...
        it("should handle a comment in the middle of text", function () {
            var nodes = parseHtmlAndCapture("Test <!-- Comment --> Test");
            expect(nodes).toEqual([
                { type: 'text', text: 'Test ', offset: 0 },
        ...
        it("should handle a comment in the middle of a tag", function () {
            var nodes = parseHtmlAndCapture("<div><!-- Comment --></div>");
            expect(nodes).toEqual([
                { type: 'openTag', tagName: 'div', offset: 0 },
                { type: 'comment', offset: 5 },
        ...
        it("should handle multiple comments", function () {
            var nodes = parseHtmlAndCapture("Test <!-- Comment --> Test <!-- Comment 2 -->");
            expect(nodes).toEqual([
                { type: 'text', text: 'Test ', offset: 0 },
                { type: 'comment', offset: 5 },
                { type: 'text', text: ' Test ', offset: 21 },
    ...
    describe('<!DOCTYPE> handling', function () {
        it('should parse a doctype tag', function () {
            var nodes = parseHtmlAndCapture("<!DOCTYPE html>");
            expect(nodes).toEqual([
                { type: 'doctype', offset: 0 }
            ]);
        });
        it('should parse a doctype tag in both upper and lower case', function () {
            var nodes = parseHtmlAndCapture("<!DOCTYPE html> and <!doctype \"blah\" \"blah blah\">");
            expect(nodes).toEqual([
                { type: 'doctype', offset: 0 },
        ...
        it('if the tag is incomplete, should treat it as text', function () {
            var nodes = parseHtmlAndCapture("<!DOCTYPE html and blah blah blah");
            expect(nodes).toEqual([
    ...
    describe('combination examples', function () {
        it('should match multiple tags of both upper and lower case with comments and doctype', function () {
            ...
            var nodes = parseHtmlAndCapture(inputStr);
            expect(nodes).toEqual([
    ...
    describe('Time Complexity - ', function () {
        ...
        it("should not freeze up the regular expression engine when presented \n\t\t\t with the input string in issue #204\n\t\t\t \n\t\t\t (Note: this is an old test that came from the days before the HTML \n\t\t\t parser was converted from a RegExp to the current state machine parser)", function () {
            var inputStr = '<img src="http://example.com/Foo" border-radius:2px;moz-border-radius:2px;khtml-border-radius:2px;o-border-radius:2px;webkit-border-radius:2px;ms-border-radius:="" 2px; "=" " class=" ">', nodes = parseHtmlAndCapture(inputStr);
            expect(nodes).toEqual([
                { type: 'text', offset: 0, text: inputStr }
            ]);
        });
