...
var url_matcher_1 = require("../../src/matcher/url-matcher");
...
describe("Autolinker.matcher.Url", function () {
    var matcher;
    beforeEach(function () {
        matcher = new url_matcher_1.UrlMatcher({
    ...
    describe('parseMatches()', function () {
        ...
        it('should return an array of a single url match when the string is the url itself', function () {
            ...
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'http://asdf.com', 0);
        });
        it('should return an array of a single url match when the url is in the middle of the string', function () {
            ...
            expect(matches.length).toBe(1);
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'http://asdf.com', 6);
        });
        ...
        it('should return an array of multiple urls when there are more than one within the string', function () {
            ...
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'http://asdf.com', 6);
            match_checker_1.MatchChecker.expectUrlMatch(matches[1], 'http://fdsa.com', 18);
        });
        it('a match within parenthesis should be parsed correctly', function () {
            var matches = matcher.parseMatches('Hello (asdf.com)');
            expect(matches.length).toBe(1);
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'http://asdf.com', 7);
        });
        it('should match an IP address', function () {
            var matches = matcher.parseMatches('http://127.0.0.1');
            expect(matches.length).toBe(1);
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'http://127.0.0.1', 0);
        });
        ...
        it('should match the entire URL with a check character', function () {
            ...
            expect(matches.length).toBe(1);
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'https://gitlab.example.com/search?utf8=✓&search=mysearch&group_id=&project_id=42&search_code=true&repository_ref=master', 0);
        });
        it('should match any local URL with numbers with http:// before', function () {
            ...
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'http://localhost.local001/test', 0);
            match_checker_1.MatchChecker.expectUrlMatch(othermatches[0], 'http://suus111.w10:8090/display/test/AI', 0);
        });
        ...
        it('should match katakana with dakuten characters (symbol with combining mark - two unicode characters)', function () {
            ...
            expect(matches.length).toBe(1);
            match_checker_1.MatchChecker.expectUrlMatch(matches[0], 'https://website.com/files/name-ボ.pdf', 0);
        });
        describe('protocol-relative URLs', function () {
            it('should match a protocol-relative URL at the beginning of the string', function () {
                var matches = matcher.parseMatches('//asdf.com');
                expect(matches.length).toBe(1);
                match_checker_1.MatchChecker.expectUrlMatch(matches[0], '//asdf.com', 0);
            });
            it('should match a protocol-relative URL in the middle of the string', function () {
                ...
                expect(matches.length).toBe(1);
                match_checker_1.MatchChecker.expectUrlMatch(matches[0], '//asdf.com', 6);
            });
